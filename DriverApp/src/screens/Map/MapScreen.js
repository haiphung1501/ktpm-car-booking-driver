        ...
        <View style={styles.container}>
          <MapView
            ...
            region={{
              latitude:
                flag === 1
                  ? (booking.driverLocation.lat + booking.pickupLocation.lat) /
                    2
                  : (booking.driverLocation.lat + booking.destination.lat) / 2,
              longitude:
                flag === 1
                  ? (booking.driverLocation.lng + booking.pickupLocation.lng) /
                    2
                  : (booking.driverLocation.lng + booking.destination.lng) / 2,
              latitudeDelta:
                flag === 1
                  ? Delta(booking.driverLocation, booking.pickupLocation) *
                    DELTA_FACTOR
                  : Delta(booking.driverLocation, booking.destination) *
                    DELTA_FACTOR,
              longitudeDelta:
                flag === 1
                  ? Delta(booking.driverLocation, booking.pickupLocation) *
            ...
            <Marker
              coordinate={
                flag === 1
                  ...
                  : {
                      latitude: booking.destination.lat,
                      longitude: booking.destination.lng,
                    }
              }>
              <DestinationView />
            </Marker>
          ...
          <Animated.View style={[styles.bottomSheet, bottomSheetAnimation]}>
            <View className="mt-3 ml-6 flex flex-row items-center"></View>
            <View className="bg-white mt-3 h-full">
                ...
                <View className="bg-white border border-slate-400 rounded-lg p-3 mt-2">
                  <View className="relative">
                    ...
                    <View className="flex flex-row gap-3 items-start">
                      ...
                      <View className="mr-6">
                        ...
                        <Text className="text-gray-500 text-base">
                          {booking.pickupAddress.fullAddress}
                        </Text>
                      </View>
                    </View>
                    <Text className="font-medium text-gray-500 ml-9 mt-4 text-base">
                      {(booking.distance / 1000).toFixed(1)}km -{' '}
                      {(booking.duration / 60).toFixed(0)} phút
                    </Text>
                    <View className="flex flex-row gap-3 items-start mt-2">
                      ...
                      <View className="mr-6">
                        <Text className="font-bold text-black text-lg">
                          {booking.destinationAddress.name}
                        </Text>
                        <Text className="text-gray-500 text-base">
                          {booking.destinationAddress.fullAddress}
                        </Text>
                      </View>
                    </View>
                  </View>
              ...
              {flag === 1 ? (
                <TouchableOpacity
                  ...
                  onPress={() => progressBooking(booking._id)}>
                  <Button className="mt-6 mb-1 p-1 rounded-lg bg-green-600 w-3/5 h-14">
                    {' '}
                ...
                <TouchableOpacity
                  className="flex items-center align-middle"
                  onPress={() => completeBooking(booking._id)}>
                  <Button className="mt-6 mb-1 p-1 rounded-lg bg-green-600 w-3/5 h-14">
                    <Text className="text-white font-bold text-lg">
                      {status[1]}
                    </Text>
